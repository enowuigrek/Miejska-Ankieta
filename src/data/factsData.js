// Dane źródłowe — jednorazowe seedowanie Firestore.
// Do odczytu w aplikacji używaj DataContext (useData()).
export const SEED_FACTS = [
    'Zielone oliwki rosną na tych samych drzewach co czarne. To te same oliwki tylko, mniej dojżałe.',

    'Trytytka, czyli opaska zaciskowa, która ma bardzo wiele zastosowań i dzięki której można naprawić prawie wszystko, swą nazwę wzięła od firmy, która wprowadziła ją na rynek - "Trytyt".',

    'Dawno temu piwo miało różne odcienie koloru brązowego. Nie było piw jasnych o złocistej barwie, ani ciemnych, gdyż produkcja słodu nie pozwalała na precyzyjne sterowanie kolorem. Było po prostu brązowe. Dlatego do dziś, brązowe oczy nazywamy piwnymi.',

    'W latach 50. zeszłego wieku w browarze Guinness wśród pracowników miał miejsce spór o to, który ptak jest najszybszym łownym ptakiem Europy. Doprowadził on do myśli, że nie ma dostępnego źródła, które mogłoby rozstrzygnąć tego typu spory, co zapoczątkowało pomysł utworzenia księgi rekordów. Tak powstała księga Guinnessa.',

    'Żaby nie piją wody. Pobierają ją przez skórę.',

    'Ciało strusia jest tak duże, że jego mózg jest mniejszy od jednego z jego oczu.',

    'Błyskawica jest 5 razy gorętsza niż powierzchnia słońca.',

    'Koliberki są jedynymi ptakami, które potrafią latać do tyłu.',

    'Woda jest jedyną substancją na Ziemi, która występuje naturalnie w trzech stanach skupienia: ciekłym, stałym i gazowym.',

    'Jedno drzewo produkuje na tyle tlenu, ile potrzeba do oddychania przez 2 osoby.',

    'Mrówki nigdy nie śpią.',

    'W 2006 roku, Nasa wysłała płyty CD ze 90 minutami muzyki Beatlesów w kosmos.',

    'Ośmiornice mają trzy serca.',

    'Najwyższa temperatura jaka kiedykolwiek została odnotowana na Ziemi to 56.7 stopni Celsjusza w Dolinie Śmierci, USA.',

    'Biała czekolada nie zawiera ziaren kakaowca. Technicznie nie powinna się nazywać czekoladą, tak jak kawa zbożowa kawą',

    'Żaden kawałek papieru nie może być złożony na pół więcej niż siedem razy. Challenge accepted?',

    'Słońce jest niebieskie z kosmosu, a nie żółte.',

    'Delfiny są jednymi z niewielu zwierząt, które potrafią rozpoznawać się nawzajem w lustrze.',

    'Jeden z najdłużej żyjących organizmów na świecie to ośmiornica o imieniu Ming, która dożyła 507 lat.',

    'Jedna trzecia żywności produkowanej na świecie jest marnowana.',

    'Koala śpi średnio 20 godzin dziennie.',

    'Niedźwiedzie polarne są leworęczne.',

    'Tęcza jest kołem pełnym, ale zazwyczaj widzimy tylko jego półokrągły fragment na niebie.',

    'Statystycznie osoby praworęczne piją więcej kawy niż osoby leworęczne.',

    'Nietoperze są jedynymi ssakami, które potrafią latać i tak naprawdę to widzą. To, że są ślepe, to mit. Używają echolokacji, ale wzroku też.',

    'Słoń to jedyny ssak, który nie potrafi skakać',

    'Nie da się kogoś uśpić chloroforem, przykłądając nasączoną nim szmatkę, czy chusteczkę, jak często widzimy w filmach. Proces działania chloroformu jako środka znieczulającego jest bardziej skomplikowany i wymaga dłuższego kontaktu oraz odpowiednich stężeń.',

    'Tłumik w pistoletach nie sprawia, że strzały stają się całkowicie bezszelestne ani niemożliwe do wykrycia. Służy głównie ochronie słuchu strzelca oraz redukcji hałasu dla osób w pobliżu. Psiu psiu w filmach to mit. Swoją drogą jak dodatkowa nasadka na lufie, miałaby wyciszyć wybuchający proch? Ona działa głównie na redukcję hałasu wytwarzanego przez gaz wydobywający się z lufy.',

    'Czerwona papryka zawiera więcej witaminy C niż cytryna. Na przykład, 100 gramów czerwonej papryki może dostarczyć około 128 mg witaminy C, podczas gdy ta sama ilość cytryny zawiera około 53 mg witaminy C.',

    'Podejmujesz około 35 000 decyzji dziennie. Większość z nich nieświadomie.',

    'Ludzkie DNA jest w 60% identyczne z DNA banana.',

    'Samotność jest statystycznie groźniejsza dla zdrowia niż palenie 15 papierosów dziennie.',

    'Na Saturnie i Jowiszu pada deszcz z diamentów.',

    'Płaczemy trzema rodzajami łez: odruchowymi, bazalnymi i emocjonalnymi. Tylko te ostatnie zawierają hormony stresu.',

    'W ciągu życia przejdziesz około 150 000 km — prawie 4 razy dookoła Ziemi.',

    'Twój mózg zużywa 20% energii ciała, choć waży tylko 2% jego masy.',

    'Przeciętny człowiek spędza 6 miesięcy życia czekając na zielone światło.',

    'Pszczoły potrafią rozpoznawać ludzkie twarze.',

    'Na świecie jest więcej telefonów komórkowych niż ludzi.',

    'Twój żołądek produkuje nową warstwę śluzu co dwa tygodnie — inaczej strawiłby sam siebie.',

    'Każda decyzja którą podjąłeś doprowadziła Cię do czytania tego zdania.',

    'Najstarsze znane drzewo na świecie ma ponad 5 000 lat i stoi w Kalifornii.',

    'Gdybyś mógł złożyć kartkę papieru 42 razy, sięgnęłaby do Księżyca.',

    'Światło docierające do nas ze Słońca ma 8 minut i 20 sekund. Widzisz przeszłość.',

    'Częstochowa leży dokładnie na 50. równoleżniku północnym — tak samo jak Praga i Kraków.',

    'Średnio spędzamy 2 lata życia na scrollowaniu telefonu.',

    'Twoje ciało zawiera wystarczająco dużo żelaza, żeby zrobić z niego gwóźdź o długości 7 cm.',

    'Koty mruczą z częstotliwością 25-150 Hz — dokładnie taką, która przyspiesza gojenie kości.',
];

// Alias dla wstecznej kompatybilności
export const FACTS_DATA = SEED_FACTS;
